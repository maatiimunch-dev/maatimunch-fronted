...
const OrderSuccess = () => {
  ...
  useEffect(() => {
    // Agar order details nahi hai to home redirect karo
    if (!orderDetails) {
      navigate('/');
    }
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 mb-16 md:mb-0">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="space-y-3 mb-4">
            {orderDetails.items.map(item => (
              <div key={item._id || item.id} className="flex gap-4 pb-3 border-b last:border-b-0">
                <img 
          ...
          <div className="border-t pt-3 space-y-2">
            <div className="flex justify-between text-gray-700">
              <span>Subtotal</span>
              <span>₹{orderDetails.total >= 1499 ? orderDetails.total : orderDetails.total - 50}</span>
            </div>
            <div className="flex justify-between text-gray-700">
              <span>Delivery Charges</span>
              <span className={orderDetails.total >= 1499 ? 'text-green-600' : ''}>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-green-50 p-3 rounded-lg mt-4">
              <p className="text-sm font-medium text-green-800">
                Payment Method: {orderDetails.paymentMethod === 'cod' ? 'Cash on Delivery' : 
                                 orderDetails.paymentMethod === 'razorpay' ? 'Razorpay (Online)' : 
                                 'Online Payment'}
              </p>
            </div>
          </div>
